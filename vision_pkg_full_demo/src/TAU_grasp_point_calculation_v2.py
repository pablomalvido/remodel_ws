...
class GraspPointCalculation():
    ...
    def exec(self, all_cables_poly, all_cables_input, index_upper, dist_per_pixel, img, analyzed_grasp_length, plot_name="", res_height=0):
        ...
        #y_limits = [max(int(y_min-(res_height*0.05)),0), min(int(y_max+(res_height*0.05)), res_height)]
        y_limits = [y_min,y_max]
        best_point, best_angle = self.gpd.exec(upper_cable_separation_poly, lower_cable_separation_poly, [min_x_eval, max_x_eval], y_limits, plot_name = plot_name, res_height=res_height)
        if int(best_point[0]) >= max_x_eval:
            best_point[0] = (max_x_eval-1)    
        elif int(best_point[0]) < min_x_eval:
            best_point[0] = min_x_eval
        if min_dist_cable_dict[int(best_point[0])] > 0 and not wrong:
            success_grasp = True
        else:
            print("Minimum distance: " + str(min_dist_cable_dict[int(best_point[0])]))
            success_grasp = False
