...
from vision_pkg_full_demo.src.TAU_grasp_point_calculation_v2_old import GraspPointCalculation
import copy
import math
import numpy as np
import time
...
rospy.init_node('DLO_detection_node_noclass')
...
class DLO_estimator():
    ...
    def __init__(self, img_path, all_colors, color_order, con_points, cable_D, con_dim, cable_lengths, pixel_D, model, grasping_point_eval_mm=[0,0], grasp_area_mm=[0,0], corner_mold=[[0,0],[0,0]], mold_size = 0, segm_opt = 0, index_upper=0, analyzed_length=500, analyzed_grasp_length=0, simplified=False):
        ...
        self.grasp_calculation = GraspPointCalculation()
    ...
    def exec(self, forward, iteration, determine_GP=False, evaluate_grasp=False, thr1_init=60, thr2_init=1.5, erosion_init=3):
        ...
        if determine_GP:
            ...
            grasp_point, success_GP = self.grasp_calculation.exec(all_cables_poly_dict, all_points_cables_dict, self.index_upper, self.mm_per_pixel, self.lines_cables_img, analyzed_grasp_length = (self.analyzed_grasp_length + self.corner_mold_top_resized[1]), plot_name=save_plot_name)
            if success_GP:
