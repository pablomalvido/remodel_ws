...
from TAU_grasp_point_calculation_v2 import GraspPointCalculation
...
class DLO_estimator():
    ...
    def __init__(self, img_path, all_colors, color_order, con_points, cable_D, con_dim, cable_lengths, pixel_D, model, grasping_point_eval_mm=[0,0], grasp_area_mm=[0,0], corner_mold=[[0,0],[0,0]], mold_size = 0, segm_opt = 0, index_upper=0, analyzed_length=500, analyzed_grasp_length=0, simplified=False):
        ...
        #Saves a plot with the img
        """
        ymin=400#400#450#200
        ymax=566#566#750#resized_img.shape[0]#1000
        xmin=160#160#150#100
        xmax=425#425#620#780

        plot_img = copy.deepcopy(resized_img[max(resized_img.shape[0]-ymax,0):resized_img.shape[0]-ymin, xmin:min(xmax,resized_img.shape[1])])
        save_img_path = os.path.join(os.path.dirname(__file__), '../imgs/temp.jpg')
        cv.imwrite(save_img_path, plot_img)
        img = plt.imread(save_img_path)
        fig, ax = plt.subplots()
        #xticks = np.arange(150, 620, 100) #wh1_zoom
        xticks = np.arange(160, 411, 50) #wh1_zoom
        ax.set_xticks(xticks)
        ax.set_aspect('equal')
        #ax.set_adjustable("datalim")
        ax.imshow(img, extent=[xmin, plot_img.shape[1]+xmin, ymin, plot_img.shape[0]+ymin])
        plt.grid(True)
        plt.savefig(os.path.dirname(os.path.realpath(__file__)) + "/../plots/adjusted/zoom_" + str(self.img_name.split('.')[0] + ".pdf"))
        #"""
    ...
    def exec(self, forward, iteration, determine_GP=False, evaluate_grasp=False, thr1_init=60, thr2_init=1.5, erosion_init=3):
        ...
        global show_imgs
        global grasp_calculation

        index = 0
        all_points_cables = []
        all_points_cables_dict = {}
        all_cables_poly_dict = {}
        grasped_cables = []
        ...
        for color in self.color_order:
            ...
            cable_line_yx, cable_line_xy, cable_poly = self.line_estimation.exec(points_cable)
            all_points_cables.append(cable_line_yx)
            all_points_cables_dict[index] = cable_line_yx
            all_cables_poly_dict[index] = cable_poly #Polynomial coefficients
        ...
        save_plot_name = "GraspPoint_" + self.img_name
        ...
        if determine_GP:
            ...
            grasp_point, success_GP = grasp_calculation.exec(all_cables_poly_dict, all_points_cables_dict, self.index_upper, self.mm_per_pixel, self.lines_cables_img, analyzed_grasp_length = (self.analyzed_grasp_length + self.corner_mold_top_resized[1]), plot_name=save_plot_name, res_height=self.lines_cables_img.shape[0])
            print(grasp_point)
            if success_GP:
                ...
                if UI_app:
                    cv.imwrite('/home/remodel/UI-REMODEL/src/assets/img/grasp_image.jpg', self.grasp_img) #Optional, saves the grasp image in the UI folder for visualization
