...
class DLO_estimator():
    ...
    def exec(self, forward, iteration, determine_GP=False, evaluate_grasp=False, thr1_init=60, thr2_init=1.5, erosion_init=3):
        ...
        if determine_GP:
            ...
            grasp_point, success_GP = grasp_calculation.exec(all_cables_poly_dict, all_points_cables_dict, self.index_upper, self.mm_per_pixel, self.lines_cables_img, analyzed_grasp_length = (self.analyzed_grasp_length + self.corner_mold_top_resized[1]), plot_name=save_plot_name)
            if success_GP:
