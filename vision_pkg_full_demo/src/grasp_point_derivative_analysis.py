...
class GraspPointDerivative():
    ...
    def plot_results_inverted(self, lines, y_limits, points=[], plot_name="plot.pdf", res_height=0):
        ...
        #plt.legend()
        plt.ylim([res_height-y_limits[1], res_height-y_limits[0]])
        plt.title('Grasp point evaluation')
        ax.set_facecolor('white')
        plt.grid(True)
        plt.savefig(os.path.dirname(os.path.realpath(__file__)) + "/../plots/" + str(plot_name.split('.')[0])+'.pdf')
        #plt.show()
        plt.clf()
    ...
    def exec(self, f_up, f_down, x_interval, y_limits, plot_name="plot.jpg", invert = True, res_height=0, plot_all=False, n_tests=30, plot_lines=False):
        ylines=[]
        x_values = np.linspace(x_interval[0], x_interval[1], 100)
        ...
        d1=self.get_derivative_dict(f_down)
        d2=self.get_derivative_dict(f_up)
        y1_values=[]
        y2_values=[]
        y_avg=[]
        avg_d_slope_values=[]
        #d_avg=[]
        for x in x_values:
            y1_values.append(self.polynomial_dict(f_down,x))
            y2_values.append(self.polynomial_dict(f_up,x))
        ...
        for i in range(init,len(x_values),step):
            if y1_values[i] > y2_values[i]:
                ...
                intersections1 = self.intersection_with_polynomial(point=[x_values[i], y_avg[i]], direction=dir, polynomial_coeff=f_down, bounds=x_interval)
                min1, min_int1 = self.get_min_intersection(point=[x_values[i], y_avg[i]], intersections=intersections1)
                intersections2 = self.intersection_with_polynomial(point=[x_values[i], y_avg[i]], direction=dir, polynomial_coeff=f_up, bounds=x_interval)
...
#"""
"""
f1=[{'c': [404.01017566711056, 0.18556081114598683], 'l': [-math.inf, math.inf]}] #Up
f2=[{'c': [393.3623697822399, 0.4291692006899363, -0.0027438000270581984, 7.611178147340695e-06, -1.1381916670592322e-08, 8.550989368430022e-12, -2.50547843512233e-15], 'l': [-math.inf, math.inf]}] #Down
x_interval = [180, 396]
y_limits = [300, 500]
gpd = GraspPointDerivative()
best_point, best_angle = gpd.exec(f1, f2, x_interval, y_limits, res_height = 1111, plot_name = 'test_new.pdf')
#"""