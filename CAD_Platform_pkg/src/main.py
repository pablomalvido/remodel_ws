...
#Initialize ROS node 
rospy.init_node('CAD_Platform_node') 
rate = rospy.Rate(10)
...
keysList = list(dict_elvez.dict_jigs.keys())
print(keysList)
        ...
        if trans.getID().getType() == 4:  #If it is a ATC station
            ...
            if(trans.getCommercial() in dict_elvez.dict_jigs):
                ...
                if ATC_temp_dict['tool_type'] == 'gripper':
                    ...
                    ATC_temp_dict['dimensions_fingers'] = [dict_elvez.dict_jigs[commercial]['finger_thickness'], dict_elvez.dict_jigs[commercial]['finger_width'], dict_elvez.dict_jigs[commercial]['finger_height']]
...
connector_info_service = 'CAD_platform/connector_info'
cable_info_service = 'CAD_platform/cable_info'
#Services for providing info about the keypoints of the ELVEZ platform
tray_info_service = 'CAD_platform/tray_info'
jig_info_service = 'CAD_platform/jig_info'
guide_info_service = 'CAD_platform/guide_info'
taping_spot_info_service = 'CAD_platform/taping_spot_info'
all_guides_service = 'CAD_platform/all_guides'
#Services for going through the operation sequence list
next_operation_service = 'CAD_platform/next_operation'
reset_sequence_list_service = 'CAD_platform/reset_sequence_list'
reset_sequence_list_to_service = 'CAD_platform/reset_sequence_list_to'
index_operation_service = 'CAD_platform/index_operation'
all_operations_service = 'CAD_platform/all_operations'
all_cad_components_service = 'CAD_platform/all_cad_components'
tool_info_service = 'CAD_platform/tool_info'
get_tools_service = 'CAD_platform/get_tools_service'
...
def all_tools_callback(req):
    resp = get_listResponse()
    resp.success = False
    for label in jigs_complete_dict:
        if label[0]=="A":
            resp.data.append(label[1:])
    resp.success = True
    return resp

rospy.Service(get_tools_service, get_list, all_tools_callback)
