...
def get_usb_cb(req):
    """
	Retrieves the list of USB devices connected to the system
	"""
    global user_name
...
def get_files_cb(req):
    """
	Retrieves the list of files inside a USB device
	"""
    global user_name
    resp = GetListResponse()
    resp.success = True
...
def move_file_cb(req):
    """
	Copies one file from an USB device into another directory
	"""
    global user_name
